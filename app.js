(() => {
  ...
  async function createEducaCityWorkbook(columns, students, context) {
    ...
    let sheet = null;
    try {
      const templateUrl = new URL('./assets/templates/educacity-planilla-base.xlsx?v=0.25.002', document.baseURI).href;
  ...
  function registerServiceWorker() {
    ...
    window.addEventListener('load', async () => {
      try {
        const registration = await navigator.serviceWorker.register('./sw.js?v=0.25.002', { updateViaCache: 'none' });
        registration.update();
