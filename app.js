(() => {
  ...
  const QUIZ_TRANSITION_SCORE_TUNE_KEY = 'encisomath:quizTransitionScoreTune:v0.24.217';
  ...
  const QUIZ_PADDING_DEBUG_KEY = 'encisomath:quizPaddingDebugTune:v0.24.217';
  ...
  function registerServiceWorker() {
    ...
    window.addEventListener('load', async () => {
      try {
        const registration = await navigator.serviceWorker.register('./sw.js?v=0.24.217', { updateViaCache: 'none' });
        registration.update();
